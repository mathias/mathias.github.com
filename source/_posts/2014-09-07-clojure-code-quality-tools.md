...
lein-ancient can also be used to find outdated lein plugins in your `~/.lein/profiles.clj` file. Just run it with the `profiles` argument:
...
That's a lot of problems for a simple file! Notice how one mistake got caught for two reasons: A misplaced docstring (placed after the arguments vector) becomes just a string in the function body which would be thrown away.

Another nice catch that Eastwood provides is detecting that the var `qux` was redefined in the file.
...
Frequently running lint tools in your project can help prevent the kind of subtle problems that come from code that looks correct but contains some small error. Eastwood is less concerned with style than tools like JSHint are, but we have other tools that cover stylistic concerns.
...
Bikeshed might give a lot of output for your existing projects, but the warnings are worth investigating and addressing. And you can always silence the long-lines warning if it doesn't matter to you with the `-m` command line argument.
...
Now, when you want to run all of these tools at once on a project, you simply have to invoke `lein omni`. I use this alias on all my Clojure(Script) projects. I have grown accustomed to seeing the kinds of output that a clean Clojure project will have.

It's worth noting that I don't typically run Eastwood unless it is necessary for the project, in which case I'd probably override the alias in the project's `project.clj` to run Eastwood as well.
...
Note that just like running the lein alias above, this may take a bit of time. Since we're piping it to `less`, it might take awhile before the output is sent to less so that you can scroll through it. While it is still running, output will periodically show up at the bottom of the `less` buffer. You can use both Emac's and vim's movement commands in `less` to advance the buffer. I find `less` to be more manageable for scrolling through output than switching to tmux's history scrolling mode.

## Managing your namespaces: [lein slamhound](https://github.com/technomancy/slamhound)

Namespace management often becomes an issue on nontrivial Clojure projects. Actively developing a project means managing the functions we pull in from other namespaces and from libraries. These require statements can often get out of date, either missing functions that must be pulled in, or containing requirements for old functions that are no longer used in the current code. [slamhound](https://github.com/technomancy/slamhound) is a tool that can help to manage dependencies in your namespaces. It knows how to require and import Clojure and Java depdendencies, accordingly, and can remove stale requires that are no longer necessary. Slamhound can often fix missing requires for functions that it can resolve.

**Note: slamhound rewrites the namespace macros in your project's .clj files!** I recommend only running it on code that's committed cleanly to your source control so that you can review and potentially roll back any changes it makes.
...
## Test coverage with [cloverage](https://github.com/lshift/cloverage)

It is often claimed that less unit testing is necessary in Clojure 


## Final Thoughts

That's all! I did write a bit more on using `lein deps :tree` to find dependency-of-dependencies conflicts, but the functionality stopped working in the most recent version of Leiningen (2.4.3) as of this writing. If you're interested in that section, I've put it online as a [gist](https://gist.github.com/mathias/2af80c34edbc53582f53). When a new release of lein brings that functionality back, I'll write a quick follow-up post for it.
...
Have fun and enjoy your cleaner codebase with these tools in your toolbelt!
